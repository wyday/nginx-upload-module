...
if ($_POST){
    ...
    echo "<tr><td>Name</td><td>Location</td><td>Content type</td><td>SHA256</td><td>Size</tr>";
	...
	for ($i=1; $i<=$slots; $i++){
		...
		if (array_key_exists($key."_name", $_POST) && array_key_exists($key."_path",$_POST)) {
			...
			$sha256 = $_POST[$key."_sha256"];
			$size = $_POST[$key."_size"];

            echo "<tr><td>$name</td><td>$tmp_name</td><td>$content_type</td><td>$sha256</td><td>$size</td>";
		}
	}
